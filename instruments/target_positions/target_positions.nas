print("*** LOADING instrument_target_positions - target_positions.nas ... ***");

# namespace : instrument_target_positions
...
#        m.debug_ligne2 = m.horizontal_container.createChild('path').setStrokeLineWidth(2).set('stroke', 'rgba(255, 0, 0, 1)').moveTo(512, 508).lineTo(512, 516);
    ...
    update: func()
    {
        var show_targets = getprop("/instrumentation/my_aircraft/target_positions/controls/enabled") or 0;
        if(show_targets == 1)
        {
            ...
            var radar_range    = getprop("/instrumentation/radar/range") or 50;

            var pitch_view_deg   = getprop("/instrumentation/my_aircraft/target_positions/view/pitch-offset-deg");
            var heading_view_deg = getprop("/instrumentation/my_aircraft/target_positions/view/heading-offset-deg");
            ...
            var targets_are_mp = getprop("/instrumentation/my_aircraft/target_positions/controls/targets_are_mp") or 0;
            if(targets_are_mp == 1)
            {
